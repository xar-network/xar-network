...
이제 `zarcli` 가 레저(Ledger) 시드를 통한 계정 키 관리를 지원합니다. 해당 기능을 이용하시기 위해서는 다음이 필요합니다:

- 이용하실 네트워크와 연결된 `zard` 인스턴스.
- 사용하시는 `zard` 인스턴스와 연동된 `zarcli` 인스턴스.
...
```bash
$ zarcli keys add {{ .Key.Name }} --ledger
