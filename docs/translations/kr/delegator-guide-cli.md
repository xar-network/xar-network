...
`xarcli`: `xarcli`는 `xard` 풀노드와 소통하기 위해 사용되는 명령어 기반 인터페이스입니다.

::: 경고
**추가적인 행동을 진행하기 전 최신 `xarcli` 클라이언트를 다운로드 하셨는지 확인하십시오**
:::

[**바이너리 설치하기**]

[**소스에서 설치하기**](https://cosmos.network/docs/xar/installation.html)

::: tip `xarcli`는 터미널 환경에서 사용됩니다. 다음과 같이 터미널을 실행하세요:
...
```bash
<<<<<<< HEAD
xarcli keys add <키 명칭 지정(YourKeyName)> --recover
=======
xarcli keys add <키_명칭(YourKeyName)> --recover
>>>>>>> b873cb16ec12163c5bf67f05e9dd6d74a0e38af5
```
...
새로운 계정을 생성하기 위해서는 `xarcli`를 설치해야합니다. 신규 계정을 생성하기 전, 프라이빗 키를 어디에 저장하고 어떻게 불러올지 미리 인지를 하셔야 합니다. 프라이빗 키를 보관하기 가장 좋은 곳은 오프라인 컴퓨터 또는 렛저 하드웨어 월렛 기기입니다. 흔히 사용되는 온라인 컴퓨터에 프라이빗 키를 보관하게 될 경우, 인터넷을 통해 컴퓨터를 침투한 공격자가 프라이빗 키를 탈취할 수 있기 때문에 상당한 리스크가 존재합니다.
...
렛저 기기를 처음 활성화할때 24개 단어로 구성된 시드키가 생성되고 기기에 저장됩니다. 렛저 기기의 시드키는 코스모스와 코스모스 계정과 호환이 되며, 해당 시드키를 기반으로 계정을 생성할 수 있습니다. 렛저 기기는 `xarcli`와 호환될 수 있게 설정이 되어야 합니다. 렛저 기기를 설정하는 방법은 다음과 같습니다:
...
```bash
<<<<<<< HEAD
xarcli keys add <키 명칭 지정(yourKeyName)> --ledger
=======
xarcli keys add <키_명칭(yourKeyName)> --ledger
>>>>>>> b873cb16ec12163c5bf67f05e9dd6d74a0e38af5
```
...
```bash
<<<<<<< HEAD
xarcli keys add <키 명칭 지정(yourKeyName)>
=======
xarcli keys add <키_명칭(yourKeyName)>
>>>>>>> b873cb16ec12163c5bf67f05e9dd6d74a0e38af5
```
...
```bash
<<<<<<< HEAD
xarcli keys add <키 명칭 지정(yourKeyName)> --recover --account 1
=======
xarcli keys add <키_명칭(yourKeyName)> --recover --account 1
>>>>>>> b873cb16ec12163c5bf67f05e9dd6d74a0e38af5
```
...
[풀노드를 운영하는 절차](https://cosmos.network/docs/xar/join-mainnet.html)와 [`xard`를 설치하는 방법](https://cosmos.network/docs/xar/installation.html)은 첨부된 링크를 확인하세요.
...
풀노드에 연결하기 위해서는 다음과 같은 형식의 주소가 필요합니다: `https://77.87.106.33:26657` (*이는 예시를 위한 주소이며 실제 풀노드 주소가 아닙니다*). 이 계정은 신뢰할 수 있는 풀노드 운영자에게서 직접 받으시기 바랍니다. 이 주소는 [다음 항목](#setting-up-xarcli)에서 사용됩니다.

## `xarcli` 설정하기

::: warning
**`xarcli`의 최신 스테이블 버전을 사용하고 있는지 확인해주세요**
:::

`xarcli`는 코스모스 허브 네트워크에서 운영되고 있는 노드와 소통할 수 있게 하는 도구입니다. 풀노드는 본인이 직접 운영하거나, 타인이 운영하는 풀노드를 사용할 수 있습니다. 이제 `xarcli`의 설정을 진행하겠습니다.

`xarcli`을 설정하기 위해서는 다음 명령어를 실행하세요:

```bash
xarcli config <플래그(flag)> <값(value)>
```
...
```bash
xarcli config node <호스트(host)>:<포트(port)>

// 예시: xarcli config node https://77.87.106.33:26657
```
...
```bash
xarcli config chain-id gos-3
```

## 블록체인 상태 조회하기

[`xarcli`](https://cosmos.network/docs/xar/xarcli.html)는 계정 잔고, 스테이킹 중인 토큰 수량, 지급 가능한 보상, 거버넌스 프로포절 등 블록체인과 관련된 모든 정보를 확인할 수 있게 합니다. 다음은 위임자에게 유용한 명령어들입니다. 다음 명령어를 실행하기 전 [xarcli 설정](#setting-up-xarcli)을 진행하세요.
...
// 검증인 목록 조회
xarcli query validators

// 검증인 주소로 (예시: cosmos10snjt8dmpr5my0h76xj48ty80uzwhraqalu4eg) 검증인 정보 조회
<<<<<<< HEAD
xarcli query validator <검증인 주소(validatorAddress)>

// 위임자 주소로 (예시: cosmos10snjt8dmpr5my0h76xj48ty80uzwhraqalu4eg) 계정의 모든 위임 기록 조회
xarcli query delegations <위임자 주소(delegatorAddress)>

// 위임자가 특정 검증인에게 위임한 기록 조회
xarcli query delegations <위임자 주소(delegatorAddress)> <검증인 주소(validatorAddress)>

// 위임자 주소로 (예시: cosmos10snjt8dmpr5my0h76xj48ty80uzwhraqalu4eg) 위임자 리워드 조회
xarcli query distribution rewards <위임자 주소(delegatorAddress)>

// 예치금(deposit)을 대기중인 모든 프로포절 조회
xarcli query proposals --status deposit_period
=======
xarcli query validator <검증인_주소(validatorAddress)>

// 위임자 주소로 (예시: cosmos10snjt8dmpr5my0h76xj48ty80uzwhraqalu4eg) 계정의 모든 위임 기록 조회
xarcli query delegations <위임자_주소(delegatorAddress)>

// 위임자가 특정 검증인에게 위임한 기록 조회
xarcli query delegations <위임자_주소(delegatorAddress)> <검증인_주소(validatorAddress)>

// 위임자 주소로 (예시: cosmos10snjt8dmpr5my0h76xj48ty80uzwhraqalu4eg) 위임자 리워드 조회
xarcli query distribution rewards <위임자_주소(delegatorAddress)>
...
// 투표가 가능한 모든 프로포절 조회
xarcli query proposals --status voting_period

// 특정 프로포절 ID로 프로포절 정보 조회
xarcli query proposal <proposalID>
```
...
::: tip **아톰을 위임하거나 위임 보상을 수령하기 전에 `xarcli`를 설치하시고 계정을 만드셔야 합니다**:::
...
xarcli tx send <수신자_주소> <보내는_수량> --from <키_이름> --gas auto --gas-adjustment 1.5 --gas-prices <가스_가격(gasPrice)>
```
...
xarcli tx staking --amount <위임할 수량(amountToBond)> --validator <검증인의 bech32 주소(bech32AddressOfValidator)> --from <위임자 키 명칭(delegatorKeyName)> --gas auto --gas-prices <가스 가격(gasPrice)>
=======
// 특정 검증인에게 아톰 위임하기
...
xarcli tx staking delegate <검증인_주소(validatorAddress> <위임할_수량(amountToBond)> --from <위임자_키_명칭(delegatorKeyName)> --gas auto --gas-adjustment 1.5 --gas-prices <가스_가격(gasPrice)>
>>>>>>> b873cb16ec12163c5bf67f05e9dd6d74a0e38af5
...
<<<<<<< HEAD
xarcli tx distribution withdraw-rewards --from <위임자 키 명칭(delegatorKeyName)>
```
=======
xarcli tx staking redelegate <기존_검증인_주소(srcValidatorAddress)> <이동할_검증인_주소(destValidatorAddress)> <재위임할_수량(amountToRedelegate)> --from <위임자_키_명칭(delegatorKeyName)> --gas auto --gas-adjustment 1.5 --gas-prices <가스_가격(gasPrice)>

// 모든 리워드 수령하기
// 플래그 값 예시: <가스_가격(gasPrice)>=0.025uatom

xarcli tx distribution withdraw-all-rewards --from <위임자_키_명칭(delegatorKeyName)> --gas auto --gas-adjustment 1.5 --gas-prices <가스_가격(gasPrice)>
...
xarcli tx staking unbond <검증인_주소(validatorAddress)> <위임_취소할_수량(amountToUnbond)> --from <위임자_키_명칭(delegatorKeyName)> --gas auto --gas-adjustment 1.5 --gas-prices <가스_가격(gasPrice)>
>>>>>>> b873cb16ec12163c5bf67f05e9dd6d74a0e38af5
...
xarcli query tx <tx 해시값(txHash)>
=======
xarcli query delegations <위임자_주소(delegatorAddress)>

// 트랜잭션이 블록체인에 포함되었으면 해당 tx 정보를 전달합니다
// 트랜잭션을 생성하셨을때 표시되었던 tx hash를 입력하세요 (트랜잭션 확인 명령어)
xarcli query tx <트랜잭션_해시(txHash)>
>>>>>>> b873cb16ec12163c5bf67f05e9dd6d74a0e38af5
...
xarcli tx gov submit-proposal --title "Test Proposal" --description "My awesome proposal" --type <프로포절 종류(type)> --deposit=10000000uftm --gas auto --gas-prices <가스 가격(gasPrice)> --from <위임자 키 명칭(delegatorKeyName)>

// 프로포절의 예치금 추가하기
// 프로포절의 proposalID 조회: $xarcli query gov proposals --status deposit_period
// 파라미터 값 예시: <예치금(deposit)>=10000000uftm

xarcli tx gov deposit <프로포절 ID(proposalID)> <추가할 예치금(deposit)> --gas auto --gas-prices <가스 가격(gasPrice)> --from <위임자 키 명칭(delegatorKeyName)>

// 프로포절에 투표하기
// 프로포절의 proposalID 조회: $xarcli query gov proposals --status voting_period
// <표 선택(option)>=yes/no/no_with_veto/abstain

xarcli tx gov vote <프로포절 ID(proposalID)> <표 선택(option)> --gas auto --gas-prices <가스 가격(gasPrice)> --from <위임자 키 명칭(delegatorKeyName)>
=======
// <프로포절_종류(type)>=text/parameter_change/software_upgrade
// 플래그 값 예시: <가스_가격(gasPrice)>=0.025uatom

xarcli tx gov submit-proposal --title "Test Proposal" --description "My awesome proposal" --type <프로포절_종류(type)> --deposit=10000000uatom --gas auto --gas-prices <가스_가격(gasPrice)> --from <위임자_키_명칭(delegatorKeyName)>

// 프로포절의 보증금 추가하기
// 프로포절의 proposalID 조회: $xarcli query gov proposals --status deposit_period
// 파라미터 값 예시: <보증금(deposit)>=10000000uatom

xarcli tx gov deposit <프로포절_ID(proposalID)> <추가할_보증금(deposit)> --gas auto --gas-prices <가스_가격(gasPrice)> --from <위임자_키_명칭(delegatorKeyName)>

// 프로포절에 투표하기
// 프로포절의 proposalID 조회: $xarcli query gov proposals --status voting_period
// <표_선택(option)>=yes/no/no_with_veto/abstain

xarcli tx gov vote <프로포절_ID(proposalID)> <표_선택(option)> --gas auto --gas-prices <가스_가격(gasPrice)> --from <위임자_키_명칭(delegatorKeyName)>
>>>>>>> b873cb16ec12163c5bf67f05e9dd6d74a0e38af5
```
...
<<<<<<< HEAD
xarcli tx staking --amount <본딩할 수량(amountToBond)> --validator <위임할 검증인의 bech32 주소(bech32AddressOfValidator)> --gas auto --gas-prices <가스 가격(gasPrice)> --generate-only > unsignedTX.json
=======
xarcli tx staking delegate <검증인_주소(validatorAddress)> <위임할_수량(amountToBond)> --from <위임자_주소(delegatorAddress)> --gas auto --gas-adjustment 1.5 --gas-prices <가스_가격(gasPrice)> --generate-only > unsignedTX.json
```
...
```bash
xarcli query account <계정_주소(yourAddress)> --chain-id cosmoshub-2
>>>>>>> b873cb16ec12163c5bf67f05e9dd6d74a0e38af5
```
...
```bash
<<<<<<< HEAD
xarcli tx sign unsignedTx.json --from <위임자 키 명칭(delegatorKeyName)> > signedTx.json
=======
xarcli tx sign unsignedTx.json --from <위임자_키_명칭(delegatorKeyName)> --offline --chain-id cosmoshub-2 --sequence <시퀀스(sequence)> --account-number <계정_번호(accountNumber)> > signedTx.json
>>>>>>> b873cb16ec12163c5bf67f05e9dd6d74a0e38af5
```
