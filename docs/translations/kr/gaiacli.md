...
`zarcli`는 코스모스 테스트넷에서 이루어지는 트랜잭션과 계정을 관리하는 커맨드 라인 인터페이스입니다. 다음은 유용할 수 있는 `zarcli` 명령어입니다:

`zarcli`의 설정 파일은 `$HOME/.zarcli/config/config.toml` 경로에 저장되며, 파일 수정 또는 `zarcli config` 명령어를 통해 수정할 수 있습니다:

```bash
zarcli config chain-id zar-9004
```

명령어 사용에 관련한 정보는 help를 참고하세요: `zarcli config --help`.

이 문서에는 유용한 `zarcli` 명령어와 예시가 포함되어있습니다.
...
- `cosmosvalconspub`
  - `zard init`로 새로운 노드가 생성될때 같이 생성되는 키.
  - `zard tendermint show-validator` 명령으로 키 값을 확인할 수 있음
...
```bash
zarcli keys add <account_name(계정/키 이름)>
```
...
```bash
zarcli keys show <account_name(계정/키 이름)>
```
...
```bash
zarcli tx send ... --fees=100photino
```

또는

```bash
zarcli tx send ... --gas-prices=1uatom
```
...
```bash
zarcli query account <account_cosmos(코스모스 주소)>
```
...
```bash
zarcli tx send <destination_cosmos(수신자 코스모스 주소)> 10faucetToken \
...
```bash
zarcli query account <account_cosmos(보낸 코스모스 계정 주소)>
zarcli query account <destination_cosmos(수신자 코스모스 주소)>
```

특정 블록 높의에서의 잔고를 확인하고 싶으시다면 `--block` 플래그를 사용하실 수 있습니다:

```bash
zarcli query account <account_cosmos(코스모스 코스모스 주소)> --block=<block_height(블록 높이)>
```

트랜잭션을 실제 전파하지 않고 시뮬레이션을 하시려면 명령어 뒤에 `--dry-run` 플래그를 추가하세요:

```bash
zarcli tx send <destination_cosmosaccaddr(받는이 계정 주소)> 10faucetToken \
...
```bash
zarcli tx send <destination_cosmosaccaddr(받는이 코스모스 주소)> 10faucetToken \
  --chain-id=<chain_id(체인 아이디)> \
  --from=<key_name(보내는 키/계정 이름)> \
...
```bash
zarcli query txs --tags='<tag(태그)>:<value(값)>'
```
...
```bash
zarcli query txs --tags='<tag(태그)>:<value(값)>' --page=1 --limit=20
```
...
- [Staking tags](https://github.com/Fantom-foundation/cosmos-sdk/blob/d1e76221d8e28824bb4791cb4ad8662d2ae9051e/x/staking/tags/tags.go#L8-L24)
- [Governance tags](https://github.com/Fantom-foundation/cosmos-sdk/blob/d1e76221d8e28824bb4791cb4ad8662d2ae9051e/x/gov/tags/tags.go#L8-L22)
- [Slashing tags](https://github.com/Fantom-foundation/cosmos-sdk/blob/d1e76221d8e28824bb4791cb4ad8662d2ae9051e/x/slashing/handler.go#L52)
- [Distribution tags](https://github.com/Fantom-foundation/cosmos-sdk/blob/develop/x/distribution/tags/tags.go#L8-L17)
- [Bank tags](https://github.com/Fantom-foundation/cosmos-sdk/blob/d1e76221d8e28824bb4791cb4ad8662d2ae9051e/x/bank/keeper.go#L193-L206)
:::
...
```bash
zarcli tx slashing unjail --from <validator-operator-addr(검증인 오퍼레이터 주소)>
```
...
```bash
zarcli query slashing signing-info <validator-pubkey(검증인 pubkey)>
```
...
```bash
zarcli query staking validator <account_cosmosval(cosmosval 계정)>
```
...
여기에서`[name]`은 `zard`를 처음 설정하셨을때 정의한 키의 명칭입니다.
...
```bash
zarcli query staking delegation <delegator_addr(위임자 코스모스 주소)> <validator_addr(검증인 주소)>
...
```bash
zarcli query staking delegation <delegator_addr(위임자 코스모스 주소)>
```
...
```bash
zarcli query staking unbonding-delegation <delegator_addr(위임자 주소)> <validator_addr(검증인 주소)>
```

또는 특정 위임자의 모든 언본딩 정보를 확인하고 싶으신 경우:

```bash
zarcli query staking unbonding-delegations <account_cosmos(위임자 주소)>
```

추가적으로 특정 검증인으로 부터 언본딩하는 정보를 확인하고 싶으신 경우:

```bash
zarcli query staking unbonding-delegations-from <account_cosmosval(검증인 cosmosval 주소)>
```
...
```bash
zarcli query staking redelegation <delegator_addr(위임자 주소)> <src_val_addr(위임 취소한 검증인 주소)> <dst_val_addr(위임 받을 검증인 주소)>
```

특정 위임자의 모든 검증인에 대한 재위임을 확인하고 싶으신 경우:

```bash
zarcli query staking redelegations <account_cosmos(위임자 코스모스 주소)>
```

특정 검증인에 대한 재위임을 확인하고 싶으신 경우:

```bash
  zarcli query staking redelegations-from <account_cosmosval(검증인 주소)>
```
...
```bash
  zarcli query delegations-to <account_cosmosval(검증인 주소)>
```
...
```bash
zarcli query gov proposer <proposal_id(프로포절 ID)>
```
...
```bash
zarcli tx gov deposit <proposal_id(프로포절 ID)> <200000000uatom(금액)> \
  --from=<name(트랜잭션을 발생시킬 키/계정 이름)> \
  --chain-id=<chain_id(체인 아이디)>
```
...
```bash
zarcli query gov deposits <proposal_id(프로포절 ID)>
```

특정 주소에 대한 보증금은 다음과 같이 확인하실 수 있습니다:

```bash
zarcli query gov deposit <proposal_id(프로포절 ID)> <depositor_address(예치자 주소)>
```
...
```bash
zarcli tx gov vote <proposal_id> <Yes/No/NoWithVeto/Abstain(표 선택)> \
  --from=<name(트랜잭션을 발생시킬 키/계정 이름)> \
  --chain-id=<chain_id(체인 아이디)>
```
...
```bash
zarcli query gov vote <proposal_id(프로포절 ID)> <voter_address(투표자 주소)>
```
과거 프로포절에 대한 표 정보를 확인하기 위해서는:

```bash
zarcli query gov votes <proposal_id(프로포절 ID)>
```
...
```bash
zarcli query gov tally <proposal_id(프로포절 ID)>
```
...
```bash
zarcli query distribution commission <validator_address(검증인 주소)>
```
...
```bash
zarcli query distribution slashes <validator_address(검증인 주소)> <start_height(시작 블록 높이)> <end_height(마지막 블록 높이)>
```
...
```bash
zarcli query distribution rewards <delegator_address(위임자 주소)> <validator_address(검증인 주소)>
```
...
```bash
zarcli query distribution rewards <delegator_address(위임자 주소)>
```
...
흔히 사용되는 `Bash`와 `Zsh` 같은 UNIX의 완료 스크립트(completion script)는 `completion` 명령어를 사용해 생성될 수 있습니다. 이 명령은 `zard`와 `zarcli`에서 사용 가능합니다.
