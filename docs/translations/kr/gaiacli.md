...
`xarcli`는 코스모스 테스트넷에서 이루어지는 트랜잭션과 계정을 관리하는 커맨드 라인 인터페이스입니다. 다음은 유용할 수 있는 `xarcli` 명령어입니다:

`xarcli`의 설정 파일은 `$HOME/.xarcli/config/config.toml` 경로에 저장되며, 파일 수정 또는 `xarcli config` 명령어를 통해 수정할 수 있습니다:

```bash
xarcli config chain-id xar-9004
```

명령어 사용에 관련한 정보는 help를 참고하세요: `xarcli config --help`.

이 문서에는 유용한 `xarcli` 명령어와 예시가 포함되어있습니다.
