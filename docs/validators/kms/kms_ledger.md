...
- Edit `addr` to point to your `xard` instance.
- Adjust `chain-id` to match your `.xard/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `xard`.
...
The KMS may complain that it cannot connect to xard. That is fine, we will fix it in the next section.
...
You need to enable KMS access by editing `.xard/config/config.toml`. In this file, modify `priv_validator_laddr` to create a listening address/port or a unix socket in `xard`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell xard the key that we've got in the previous section.

```bash
xard gentx --name kmsval --pubkey {.ValidatorKey} 
```

Now start `xard`. You should see that the KMS connects and receives a signature request.
