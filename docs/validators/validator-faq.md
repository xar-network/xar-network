...
- **Lachesis Key**: This is a unique key used to sign consensus votes.
  - It is associated with a public key `zarvalconspub` (Get this value with `zard tendermint show-validator`)
  - It is generated when the node is created with zard init.
- **Application key**: This key is created from `zarcli` and used to sign transactions. Application keys are associated with a public key prefixed by `zarpub` and an address prefixed by `zar`. Both are derived from account keys generated by `zarcli keys add`.

Note: A validator's operator key is directly tied to an application key, but
uses reserved prefixes solely for this purpose: `zarvaloper` and `zarvaloperpub`
...
>>>>>>> f41a660cdd5bea173139965ade55bd25d1ee3429
