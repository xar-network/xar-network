...
- `cosmos`
  - Derived from account keys generated by `xarcli keys add`
...
- `cosmosvalconspub`
  - Generated when the node is created with `xard init`.
...
When broadcasting transactions, `xarcli` accepts a `--broadcast-mode` flag. This
...
```bash
xarcli tx send <sender_key_name_or_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id>
```
...
```bash
xarcli tx send <sender_key_name_or_address> <destination_cosmosaccaddr> 10faucetToken \
  --chain-id=<chain_id> \
  --dry-run
```
...
```bash
xarcli tx send <sender_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id> \
...
::: tip Note
The `--generate-only` flag prevents `xarcli` from accessing the local keybase.
...
```bash
xarcli query minting params
```

To query for the current inflation value:

```bash
xarcli query minting inflation
```

To query for the current annual provisions value:

```bash
xarcli query minting annual-provisions
```
...
where `[name]` is the name of the key you specified when you initialized `xard`.
...
```bash
xarcli query distribution community-pool
```
...
```bash
xarcli tx send cosmos1570v2fq3twt0f0x02vhxpuzc9jc4yl30q2qned 1000000uftm \
