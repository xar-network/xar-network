package auction

import (
	"testing"

	sdk "github.com/cosmos/cosmos-sdk/types"
	"github.com/stretchr/testify/require"
)

// TODO can this be less verbose? Should PlaceBid() be split into smaller functions?
// It would be possible to combine all auction tests into one test runner.
func TestForwardAuction_PlaceBid(t *testing.T) {
	seller := sdk.AccAddress([]byte("a_seller"))
	buyer1 := sdk.AccAddress([]byte("buyer1"))
	buyer2 := sdk.AccAddress([]byte("buyer2"))
	end := endTime(10000)
	now := endTime(10)

	type args struct {
		currentBlockHeight endTime
		bidder             sdk.AccAddress
		lot                sdk.Coin
		bid                sdk.Coin
	}
	tests := []struct {
		name            string
		auction         ForwardAuction
		args            args
		expectedOutputs []bankOutput
		expectedInputs  []bankInput
		expectedEndTime endTime
		expectedBidder  sdk.AccAddress
		expectedBid     sdk.Coin
		expectpass      bool
	}{
		{
			"normal",
			ForwardAuction{BaseAuction{
				Initiator:  seller,
				Lot:        c("csdt", 100),
				Bidder:     buyer1,
				Bid:        c("ftm", 6),
				EndTime:    end,
				MaxEndTime: end,
			}},
			args{now, buyer2, c("csdt", 100), c("ftm", 10)},
			[]bankOutput{{buyer2, c("ftm", 10)}},
			[]bankInput{{buyer1, c("ftm", 6)}, {seller, c("ftm", 4)}},
			now + BidDuration,
			buyer2,
			c("ftm", 10),
			true,
		},
		{
			"lowBid",
			ForwardAuction{BaseAuction{
				Initiator:  seller,
				Lot:        c("csdt", 100),
				Bidder:     buyer1,
				Bid:        c("ftm", 6),
				EndTime:    end,
				MaxEndTime: end,
			}},
			args{now, buyer2, c("csdt", 100), c("ftm", 5)},
			[]bankOutput{},
			[]bankInput{},
			end,
			buyer1,
			c("ftm", 6),
			false,
		},
		{
			"equalBid",
			ForwardAuction{BaseAuction{
				Initiator:  seller,
				Lot:        c("csdt", 100),
				Bidder:     buyer1,
				Bid:        c("ftm", 6),
				EndTime:    end,
				MaxEndTime: end,
			}},
			args{now, buyer2, c("csdt", 100), c("ftm", 6)},
			[]bankOutput{},
			[]bankInput{},
			end,
			buyer1,
			c("ftm", 6),
			false,
		},
		{
			"timeout",
			ForwardAuction{BaseAuction{
				Initiator:  seller,
				Lot:        c("csdt", 100),
				Bidder:     buyer1,
				Bid:        c("ftm", 6),
				EndTime:    end,
				MaxEndTime: end,
			}},
			args{end + 1, buyer2, c("csdt", 100), c("ftm", 10)},
			[]bankOutput{},
			[]bankInput{},
			end,
			buyer1,
			c("ftm", 6),
			false,
		},
		{
			"hitMaxEndTime",
			ForwardAuction{BaseAuction{
				Initiator:  seller,
				Lot:        c("csdt", 100),
				Bidder:     buyer1,
				Bid:        c("ftm", 6),
				EndTime:    end,
				MaxEndTime: end,
			}},
			args{end - 1, buyer2, c("csdt", 100), c("ftm", 10)},
			[]bankOutput{{buyer2, c("ftm", 10)}},
			[]bankInput{{buyer1, c("ftm", 6)}, {seller, c("ftm", 4)}},
			end, // end time should be capped at MaxEndTime
			buyer2,
			c("ftm", 10),
			true,
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			// update auction and return in/outputs
			outputs, inputs, err := tc.auction.PlaceBid(tc.args.currentBlockHeight, tc.args.bidder, tc.args.lot, tc.args.bid)

			// check for err
			if tc.expectpass {
				require.Nil(t, err)
			} else {
				require.NotNil(t, err)
			}
			// check for correct in/outputs
			require.Equal(t, tc.expectedOutputs, outputs)
			require.Equal(t, tc.expectedInputs, inputs)
			// check for correct endTime, bidder, bid
			require.Equal(t, tc.expectedEndTime, tc.auction.EndTime)
			require.Equal(t, tc.expectedBidder, tc.auction.Bidder)
			require.Equal(t, tc.expectedBid, tc.auction.Bid)
		})
	}
}

func TestReverseAuction_PlaceBid(t *testing.T) {
	buyer := sdk.AccAddress([]byte("a_buyer"))
	seller1 := sdk.AccAddress([]byte("seller1"))
	seller2 := sdk.AccAddress([]byte("seller2"))
	end := endTime(10000)
	now := endTime(10)

	type args struct {
		currentBlockHeight endTime
		bidder             sdk.AccAddress
		lot                sdk.Coin
		bid                sdk.Coin
	}
	tests := []struct {
		name            string
		auction         ReverseAuction
		args            args
		expectedOutputs []bankOutput
		expectedInputs  []bankInput
		expectedEndTime endTime
		expectedBidder  sdk.AccAddress
		expectedLot     sdk.Coin
		expectpass      bool
	}{
		{
			"normal",
			ReverseAuction{BaseAuction{
				Initiator:  buyer,
				Lot:        c("ftm", 10),
				Bidder:     seller1,
				Bid:        c("csdt", 100),
				EndTime:    end,
				MaxEndTime: end,
			}},
			args{now, seller2, c("ftm", 9), c("csdt", 100)},
			[]bankOutput{{seller2, c("csdt", 100)}},
			[]bankInput{{seller1, c("csdt", 100)}, {buyer, c("ftm", 1)}},
			now + BidDuration,
			seller2,
			c("ftm", 9),
			true,
		},
		{
			"highBid",
			ReverseAuction{BaseAuction{
				Initiator:  buyer,
				Lot:        c("ftm", 10),
				Bidder:     seller1,
				Bid:        c("csdt", 100),
				EndTime:    end,
				MaxEndTime: end,
			}},
			args{now, seller2, c("ftm", 11), c("csdt", 100)},
			[]bankOutput{},
			[]bankInput{},
			end,
			seller1,
			c("ftm", 10),
			false,
		},
		{
			"equalBid",
			ReverseAuction{BaseAuction{
				Initiator:  buyer,
				Lot:        c("ftm", 10),
				Bidder:     seller1,
				Bid:        c("csdt", 100),
				EndTime:    end,
				MaxEndTime: end,
			}},
			args{now, seller2, c("ftm", 10), c("csdt", 100)},
			[]bankOutput{},
			[]bankInput{},
			end,
			seller1,
			c("ftm", 10),
			false,
		},
		{
			"timeout",
			ReverseAuction{BaseAuction{
				Initiator:  buyer,
				Lot:        c("ftm", 10),
				Bidder:     seller1,
				Bid:        c("csdt", 100),
				EndTime:    end,
				MaxEndTime: end,
			}},
			args{end + 1, seller2, c("ftm", 9), c("csdt", 100)},
			[]bankOutput{},
			[]bankInput{},
			end,
			seller1,
			c("ftm", 10),
			false,
		},
		{
			"hitMaxEndTime",
			ReverseAuction{BaseAuction{
				Initiator:  buyer,
				Lot:        c("ftm", 10),
				Bidder:     seller1,
				Bid:        c("csdt", 100),
				EndTime:    end,
				MaxEndTime: end,
			}},
			args{end - 1, seller2, c("ftm", 9), c("csdt", 100)},
			[]bankOutput{{seller2, c("csdt", 100)}},
			[]bankInput{{seller1, c("csdt", 100)}, {buyer, c("ftm", 1)}},
			end, // end time should be capped at MaxEndTime
			seller2,
			c("ftm", 9),
			true,
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			// update auction and return in/outputs
			outputs, inputs, err := tc.auction.PlaceBid(tc.args.currentBlockHeight, tc.args.bidder, tc.args.lot, tc.args.bid)

			// check for err
			if tc.expectpass {
				require.Nil(t, err)
			} else {
				require.NotNil(t, err)
			}
			// check for correct in/outputs
			require.Equal(t, tc.expectedOutputs, outputs)
			require.Equal(t, tc.expectedInputs, inputs)
			// check for correct endTime, bidder, bid
			require.Equal(t, tc.expectedEndTime, tc.auction.EndTime)
			require.Equal(t, tc.expectedBidder, tc.auction.Bidder)
			require.Equal(t, tc.expectedLot, tc.auction.Lot)
		})
	}
}

func TestForwardReverseAuction_PlaceBid(t *testing.T) {
	csdtOwner := sdk.AccAddress([]byte("a_csdt_owner"))
	seller := sdk.AccAddress([]byte("a_seller"))
	buyer1 := sdk.AccAddress([]byte("buyer1"))
	buyer2 := sdk.AccAddress([]byte("buyer2"))
	end := endTime(10000)
	now := endTime(10)

	type args struct {
		currentBlockHeight endTime
		bidder             sdk.AccAddress
		lot                sdk.Coin
		bid                sdk.Coin
	}
	tests := []struct {
		name            string
		auction         ForwardReverseAuction
		args            args
		expectedOutputs []bankOutput
		expectedInputs  []bankInput
		expectedEndTime endTime
		expectedBidder  sdk.AccAddress
		expectedLot     sdk.Coin
		expectedBid     sdk.Coin
		expectpass      bool
	}{
		{
			"normalForwardBid",
			ForwardReverseAuction{BaseAuction: BaseAuction{
				Initiator:  seller,
				Lot:        c("xrp", 100),
				Bidder:     buyer1,
				Bid:        c("csdt", 5),
				EndTime:    end,
				MaxEndTime: end},
				MaxBid:      c("csdt", 10),
				OtherPerson: csdtOwner,
			},
			args{now, buyer2, c("xrp", 100), c("csdt", 6)},
			[]bankOutput{{buyer2, c("csdt", 6)}},
			[]bankInput{{buyer1, c("csdt", 5)}, {seller, c("csdt", 1)}},
			now + BidDuration,
			buyer2,
			c("xrp", 100),
			c("csdt", 6),
			true,
		},
		{
			"normalSwitchOverBid",
			ForwardReverseAuction{BaseAuction: BaseAuction{
				Initiator:  seller,
				Lot:        c("xrp", 100),
				Bidder:     buyer1,
				Bid:        c("csdt", 5),
				EndTime:    end,
				MaxEndTime: end},
				MaxBid:      c("csdt", 10),
				OtherPerson: csdtOwner,
			},
			args{now, buyer2, c("xrp", 99), c("csdt", 10)},
			[]bankOutput{{buyer2, c("csdt", 10)}},
			[]bankInput{{buyer1, c("csdt", 5)}, {seller, c("csdt", 5)}, {csdtOwner, c("xrp", 1)}},
			now + BidDuration,
			buyer2,
			c("xrp", 99),
			c("csdt", 10),
			true,
		},
		{
			"normalReverseBid",
			ForwardReverseAuction{BaseAuction: BaseAuction{
				Initiator:  seller,
				Lot:        c("xrp", 99),
				Bidder:     buyer1,
				Bid:        c("csdt", 10),
				EndTime:    end,
				MaxEndTime: end},
				MaxBid:      c("csdt", 10),
				OtherPerson: csdtOwner,
			},
			args{now, buyer2, c("xrp", 90), c("csdt", 10)},
			[]bankOutput{{buyer2, c("csdt", 10)}},
			[]bankInput{{buyer1, c("csdt", 10)}, {csdtOwner, c("xrp", 9)}},
			now + BidDuration,
			buyer2,
			c("xrp", 90),
			c("csdt", 10),
			true,
		},
		// TODO more test cases
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			// update auction and return in/outputs
			outputs, inputs, err := tc.auction.PlaceBid(tc.args.currentBlockHeight, tc.args.bidder, tc.args.lot, tc.args.bid)

			// check for err
			if tc.expectpass {
				require.Nil(t, err)
			} else {
				require.NotNil(t, err)
			}
			// check for correct in/outputs
			require.Equal(t, tc.expectedOutputs, outputs)
			require.Equal(t, tc.expectedInputs, inputs)
			// check for correct endTime, bidder, bid
			require.Equal(t, tc.expectedEndTime, tc.auction.EndTime)
			require.Equal(t, tc.expectedBidder, tc.auction.Bidder)
			require.Equal(t, tc.expectedLot, tc.auction.Lot)
			require.Equal(t, tc.expectedBid, tc.auction.Bid)
		})
	}
}

// defined to avoid cluttering test cases with long function name
func c(denom string, amount int64) sdk.Coin {
	return sdk.NewInt64Coin(denom, amount)
}
