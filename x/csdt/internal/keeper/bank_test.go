...
func TestKeeper_AddSubtractGetCoins(t *testing.T) {
	...
	tests := []struct {
		...
		{"addLiquidatorStable", LiquidatorAccountAddress, true, cs(c(types.StableDenom, 53)), cs(c(types.StableDenom, 153))},
		{"subLiquidatorStable", LiquidatorAccountAddress, false, cs(c(types.StableDenom, 53)), cs(c(types.StableDenom, 47))},
		{"addLiquidatorGov", LiquidatorAccountAddress, true, cs(c(types.GovDenom, 53)), cs(c(types.StableDenom, 100))},  // no change to balance
		{"subLiquidatorGov", LiquidatorAccountAddress, false, cs(c(types.GovDenom, 53)), cs(c(types.StableDenom, 100))}, // no change to balance
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			// setup k
			mapp, k, _, _ := setUpMockAppWithoutGenesis()
			...
			k.SetLiquidatorModuleAccount(ctx, LiquidatorModuleAccount{Coins: cs(c(types.StableDenom, 100))}) // set gov coin "balance" to zero
