...
func TestYearHourlyAccrual(t *testing.T) {
	...
	for i := 0; i < 365*24; i++ {
		accum, minted = calculateInterest(accum, supply, annualInterest, lastAccrual, lastAccrual.Add(time.Hour))
...
func TestRandomBlockTimes(t *testing.T) {
	...
	for {
		...
		accum, minted = calculateInterest(accum, supply, annualInterest, lastAccrual, lastAccrual.Add(d))
...
func TestMultipleCoinsAccrual(t *testing.T) {
	...
	state := NewInterestState("credit", "0.001", "buck", "0.03")
	...
	for i := 0; i < 365*24*60; i++ {
		...
		mintedCoins := applyInterest(&state, supply, currentTime)
...
func TestStartTimeInFuture(t *testing.T) {
	...
	assert.Equal(t, sdk.ZeroDec(), state.InterestAssets[0].Accum)
	assert.Equal(t, sdk.ZeroDec(), state.InterestAssets[1].Accum)
}
