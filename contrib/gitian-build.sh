...
DEFAULT_XAR_SIGS=${XAR_SIGS:-'xar.sigs'}
DEFAULT_GITIAN_REPO='https://github.com/tendermint/gitian-builder'
DEFAULT_GBUILD_FLAGS=''
DEFAULT_SIGS_REPO='https://github.com/xar-network/xar-network.sigs'
...
The signature will be saved in '${DEFAULT_XAR_SIGS}/'. An alternative output directory
for signatures can be supplied via the environment variable \$XAR_SIGS.
...
f_build() {
  ...
  bin/gbuild --commit xar="$g_commit" ${GBUILD_FLAGS} "$l_descriptor"
...
g_sigs_dir=${XAR_SIGS:-"${g_workdir}/${DEFAULT_XAR_SIGS}"}
